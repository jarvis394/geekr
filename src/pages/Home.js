...
const PostItem = ({ post }) => {
  ...
  const bottomRow = [
    { icon: <ThumbsUpDownIcon className={classes.postBottomRowItemIcon} />, text: score },
    { icon: <VisibilityIcon className={classes.postBottomRowItemIcon} />, text: reads },
    { icon: <BookmarkIcon className={classes.postBottomRowItemIcon} />, text: favorites },
    { icon: <ChatBubbleIcon className={classes.postBottomRowItemIcon} />, text: comments },
  ]
      ...
      <Grid container>
        ...
        <Grid className={classes.postBottomRow} container xs={12}>
          {bottomRow.map(({ icon, text }, i) => (
            ...
            >
              {icon}
              <Typography style={{ fontSize: 12, fontWeight: 600 }}>{text}</Typography>
            </Grid>
          ))}
        </Grid>
      </Grid>
